...
public class OVTestHandler : TestHandler
{

    public List<OVTestItem> testItemFrontEnd;
    public OVTestItem frontEndItem;
    ...
    public void LoadNextTest()
    {
        if (!testItemFrontEnd[currentTestNumber].skipPrint)
            OVOutputHandler.StopTimer(testAbbrev, testItemFrontEnd[currentTestNumber].id);
        DeletePrevTest();
        currentTestNumber++;
        //currEvent = 0;
    ...
    public void SortItems()
    {
        ...
        foreach (OVTestItem l in testItemFrontEnd)
        {
            posValues.Add(l.pos);
            print(l.pos);
        }
