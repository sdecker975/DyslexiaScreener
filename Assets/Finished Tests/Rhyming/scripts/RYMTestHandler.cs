...
public class RYMTestHandler : TestHandler
{

    public List<RYMTestItem> testItemFrontEnd;
    public RYMTestItem frontEndItem;
    ...
    public void CreateTest(RYMTestItem t)
    {
        ...
        for (int i = 0; i < t.LetterButtons.Length; i++)
        {
            ...
            o.GetComponent<ClickCardRYM>().isCorrect = t.isCorrect[i];
            o.GetComponent<ClickCardRYM>().responsePosition = i + 1;
            o.GetComponent<ClickCardRYM>().responseName = t.wordPictures[i].name;
    ...
    public void LoadNextTest()
    {
        if (!testItemFrontEnd[currentTestNumber].skipPrint)
            RYMOutputHandler.StopTimer(testAbbrev, testItemFrontEnd[currentTestNumber].id);
        DeletePrevTest();
        currentTestNumber++;
        //currEvent = 0;
    ...
    public void SortItems()
    {
        ...
        foreach (RYMTestItem l in testItemFrontEnd)
        {
            posValues.Add(l.pos);
            print(l.pos);
        }
