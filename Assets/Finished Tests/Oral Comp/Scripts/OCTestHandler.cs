...
public class OCTestHandler : TestHandler
{
    public List<OCTestItem> testItemFrontEnd;
    public OCTestItem frontEndItem;
    ...
    public void CreateTest(OCTestItem t)
    {
        ...
        for (int i = 0; i < t.LetterButtons.Length; i++)
        {
            ...
            o.GetComponent<ClickCardOC>().isCorrect = t.isCorrect[i];
            o.GetComponent<ClickCardOC>().responsePosition = i + 1;
            o.GetComponent<ClickCardOC>().responseName = t.wordPictures[i].name;
    ...
    public void LoadNextTest()
    {
        if (!testItemFrontEnd[currentTestNumber].skipPrint)
            OCOutputHandler.StopTimer(testAbbrev, testItemFrontEnd[currentTestNumber].id);
        DeletePrevTest();
        currentTestNumber++;
        //currEvent = 0;
    ...
    public void SortItems()
    {
        ...
        foreach (OCTestItem l in testItemFrontEnd)
        {
            posValues.Add(l.pos);
            print(l.pos);
        }
