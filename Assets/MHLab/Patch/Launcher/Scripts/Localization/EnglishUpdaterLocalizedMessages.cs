...
namespace MHLab.Patch.Launcher.Scripts.Localization
{
    public sealed class EnglishUpdaterLocalizedMessages : IUpdaterLocalizedMessages
    {
        ...
        public string UpdateCheckedFile     => "Checked {0}";
        public string UpdateFixedAttributes => "Fixed attributes for {0}";
        public string UpdateFixedMetadata   => "Fixed metadata for {0}";
        public string UpdateDownloadingFile => "Downloading {0}";
        public string UpdateRepairedFile    => "Repaired {0}";

        public string NotAvailableNetwork =>
            "Network is not available or connectivity is low/weak... Check your connection!";

        public string NotAvailableServers => "Our servers are not responding... Wait some minutes and retry!";

        public string LogsFileNotWritable =>
            "Cannot write to the logs file. Probably the Launcher has not sufficient privileges. Continue anyway?";

        public string UpdateProcessProgressing => "Updating process in progress...";
